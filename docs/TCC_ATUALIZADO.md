...
Essa elevada similaridade tem implicações práticas para a conservação: marcadores mitocondriais baseados em regiões conservadas (como COX1 e CYTB) podem não ter resolução suficiente para distinguir espécimes das duas espécies em análises forenses. A disponibilidade de mitogenomas completos de ambas permite identificar regiões com maior variabilidade (como a região controle/D-loop) que podem servir como marcadores diagnósticos mais informativos.
